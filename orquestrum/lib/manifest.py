...
def _render_managed(state: ManifestState) -> str:
    ...
    lines = [
        ...
        '',
        'Orquestrum orchestrates agent-driven development in this project. Integration',
        'files live under tool-specific directories (`.claude/`, `.opencode/`, etc.) plus',
        '`.sdd/` for governance docs. Per-project metrics live in `.orquestrum/metrics/`',
        '(gitignored). Governance documentation is under `.sdd/docs/agent-context/`',
        '(canonical rules) and `.sdd/docs/governance/` (operator-facing).',
        '',
        '## History',
        '',
    ]
